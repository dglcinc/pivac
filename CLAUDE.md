...
- **Chiltrix CX75 Modbus/BMS port — terminals and serial settings (2026-08-22)**: the CX's dedicated BMS terminals are **`DA1` = A ("+") and `DA2` = B ("−")**, separate from the wired controller's bus, and the chiller is a **slave** there so an external master may poll on demand. Settings from Chiltrix's own ProtoAir gateway guide: **Modbus RTU, 9600 / none / 8 / 1, Node-ID `1`** for a single chiller. `DA1`/`DA2` identified from the CX65 hi-res wiring diagram, whose terminal strip reads `DA1 DA2 0V B3 A3 B2 A2 B1 A1 GND +12V` with "PC RS485" labelling that area; the numbered pairs are the wired controller (`485A` orange / `485B` blue) and the sensor buses (the CX50-2 IOM puts the DHC humidity sensor on `A2/B2`). **Chiltrix publishes no CX75 manual** — the CX65 is the closest sibling. Four Chiltrix PDFs are now filed in `~/OneDrive - DGLC/Claude/HVAC Manuals/`: `CX50-2-IOM.pdf`, `Remote-Gateway-BACnet-Guide-rev2.pdf`, `cx65-1-iom.pdf`, `cx65-1-hi-res-wiring-diagram.pdf`. **⚠️ The community register maps contradict each other** — jasipsw (CX50-2) and gonzojive (CX34) disagree on nearly every address, and neither covers a CX75, so every register must be verified against the panel before use. Both maps and the verification recipe are in `docs/unico-cooling-assessment-and-tuning.md` §4.2 (PR #117, still unmerged).
