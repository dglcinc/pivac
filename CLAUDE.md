...
- **Sprinkler flow CALIBRATED 2026-06-16**: `pivac.Sprinkler` computes irrigation flow as `(flcrt/flwrt)*fpr*60*flow_scale` gal/min from the OpenSprinkler `/jc` API. Calibrated against the iPerl meter. First 2-min-zone calibration (17.20 gal / 3280 pulses → 0.00524) was **contaminated by concurrent household use** (came out ~2× high). **Recalibrated 2026-06-17 to `fpr = 0.0025`** from a cleaner overnight figure: **1042 gal meter movement / 416046 overnight-irrigation pulses** (`/jl`). Still approximate (the 1042 may include some overnight household use) — refine with a truly-isolated run later. Set in `/etc/pivac/config.yml`; `flow_scale` stays 1.0. The OS-reported `fpr` from `/jo` was **0.07 (≈13× too high)** — and OpenSprinkler's own `fpr` field has only 0.01 resolution so it *cannot represent* 0.00524, which is why the OS app's flow is also wrong; the pivac float override is the correct place. `/jl` flow-log entry format = `[pulse_count, "fl", duration_s, end_unixtime]`. Auth is **md5 of the OS *device* password** (`/jc` returns `{"result":2}` when wrong) — **not** the nginx `/sprinkler/` Basic-Auth login; the device password is set in the OpenSprinkler app → Edit Options → Advanced. To recalibrate: clean zone run, `(after−before gal)/pulses`.
- **WaterMeter (domestic iPerl) reader is NOT yet reliable — needs better lighting (status 2026-06-16 night)**: full **0–9 binary template library** built from overnight collector frames (`/etc/pivac/wm-templates/`), `_prep_cell` **binarizes** cells (Otsu) so digit shape (not LCD texture) drives the match, and the monotonic guard now **self-heals** (`rebase_after`=4 consecutive consistent lower reads re-baseline, fixing the over-read lock-up). On the *trained* value range it reads ~24/26, but it **does not generalize**: when the meter advances into digit-position patterns not in the training set (e.g. a `7` in the thousands place after a high-use day) it **misreads** (observed: true `627713` read as `627177`), because each digit position has different lighting/contrast. Segment-decode was tried and fails (0/26 — the LCD font + low contrast don't yield clean segments). **Root cause = low LCD contrast in the pit.** The fix is the planned **visual assistance: stronger/more-even pit lighting** (David's offer), which would enable either generalizing templates or training-free exact 7-segment decode. Until then the reader publishes unreliable values — **consider it provisional.** Decimals/`.flowing` still uncalibrated (integer-gallon). The **irrigation** side (`pivac.Sprinkler`) is unaffected and accurate. Config: `min_corr` 0.33, `rebase_after` 4. Warp corners + digit boxes in `/etc/pivac/config.yml`; camera mode/lighting must stay locked or recalibrate.
