...
| pivac-chiltrix          | pivac.ChiltrixModbus    | **Chiltrix CX75** heat pump, 184 Modbus addresses | RS-485 → UNO R4 on USB |
...
- `scripts/systemd/grafana-graph-bridge.service` — runs as user `pi`, `EnvironmentFile=-/etc/pivac/graph.env`, `Restart=always`. A firing alert labelled `source: sentry` also makes the bridge run `sentry-warp-search.py --capture 300 --eyecheck` and mail the report and image as a follow-up (`SENTRY_EYECHECK_*` env overrides in the unit; an empty `SENTRY_EYECHECK_CMD` disables it).
...
**Rule files** live in `grafana/provisioning/alerting/` (`redlink-stale`, `sensor-freshness`, `domestic-water`, `sentry-boiler`, `chiltrix`, `zone-setpoints`). **Keep the zone setpoints within 2 °F of each other**: the house is open plan, so a zone set apart conditions its neighbours (the kids room at 73 °F beside a 76 °F family room ran 1.5 times its duty at 74 °F and called for 11 hours on 2026-09-16; `docs/unico-cooling-assessment-and-tuning.md` Appendix J). `zone-coolset-spread` and `zone-heatset-spread` (info, in `zone-setpoints.yaml`) fire when any two of the five RedLink setpoints differ by more than 2 °F for 2 h, gated on `HPCOOL` and `HPHEAT`, and mail once a day while it stands. `hz432-mode-changeover` (in `chiltrix.yaml`, info) fires once per heat/cool changeover of the HZ-432, read from `HPHEAT.statenum`, to count how often a zone forces the other mode; the panel cannot prevent the old mode's water reaching that zone. **The aquastat interlock is deferred on measurement (2026-09-22, plan §7.1):** over the eight changeovers of 09-08 to 09-22 a calling zone took wrong-side water for 3 to 25 minutes each, 17 kBTU in all (about 1 kWh for the chiller to take back), and no thermostat moved a degree during any of it, while three of the four episodes had the kids room calling cool against a master heat call on the same loop, which the interlock cannot help. Revisit when changeovers become daily, or when a thermostat moves a full degree inside a wrong-water window. If it is warranted the part is the Inkbird ITC-1000 at $20; plan §7.1 prices the alternatives. The interlock as designed is plan §7.1 (L6006C1018 on the primary supply at ~70 °F, `R`-`B` through `HPCOOL` pole 3, `R`-`W` through `HPHEAT` pole 3 and through `DHWX` pole 3, and an unconditional `BLR` path, all in parallel into the `ZV` relay's coil, because the panel releases `B` on a boiler call and the boiler loop bypasses the tank; pole 1's spare terminal on either relay shares the Chiltrix contact's common and must not be used); while changeovers stay rare, thermostat programming is the remedy. A power interruption to the boiler-room circuit fires it on the panel's return (2026-09-19, four hours dark), so drop those firings from the count. Every rule routes to `graph-bridge`; thresholds, runbooks and retired-rule evidence are in `docs/grafana-alerting-notes.md`.
...
sudo chown root:grafana /etc/grafana/provisioning/alerting/{contact-points,redlink-stale,sensor-freshness,domestic-water,sentry-boiler,chiltrix,zone-setpoints}.yaml
sudo chmod 640         /etc/grafana/provisioning/alerting/{contact-points,redlink-stale,sensor-freshness,domestic-water,sentry-boiler,chiltrix,zone-setpoints}.yaml
...
> **Image sizing and exclusions:** `image-backup` never resizes an existing image on incrementals, so `pivac.img` was grown to the full card size (the image's MBR disk identifier must equal the live card's, `0x059be283` since the 2026-09-07 cutover, because an incremental copies the card's `cmdline.txt` and `fstab` verbatim and `image-backup` only rewrites their `PARTUUID=` in extract mode; after a card change set it with `sfdisk --disk-id pivac.img 0x<id>` before the next run), and the script passes `-o '--exclude=/home/pi/thinclient_drives'` for the xrdp FUSE mount that root cannot traverse. Both were the 2026-06-01 first-run failures; details in `docs/operational-notes.md`.

`sd-clone.timer` runs `sd-clone.service` weekly (Sunday 02:00 EDT). The service runs `scripts/sd-clone.sh`, which auto-discovers the populated slot of the USB SD reader by USB VID:PID `05e3:0764` (Anker USB 3.0 Micro SD Card Reader, Genesys Logic chipset), refuses if the target matches the booted disk, then calls `rpi-clone <target> -U` and **rewrites `root=PARTUUID=` in the clone's `/boot/firmware/cmdline.txt` to the target's disk identifier** (billw2's rpi-clone only looks for `/boot/cmdline.txt`, so on Bookworm every clone before 2026-09-07 was unbootable: kernel loads, then waits for the old card's root partition). The script exits 2 if that rewrite cannot be verified. No service stop — `rpi-clone` is designed for live cloning. First run repartitions and takes ~30 min; subsequent incrementals are ~3 min. The clone is a directly bootable hot-recovery spare: pull live SD, drop the spare in, reboot. **The USB reader must be on the live Pi with a card in it**; after the cutover the new Pi has neither yet. Install dependency: `rpi-clone` from `~/github/rpi-clone` (billw2/rpi-clone), copied to `/usr/local/sbin/`.
...
- **Register map:** 140 on/off, 141 mode, 142 cooling target (whole °C), 202 ambient, 205 outlet, 281 inlet (all ÷10 °C), 213 flow (÷10 L/min), 227 compressor Hz, 256 current (÷10 A). `inlet` is water going TO the chiller and `outlet` water returning FROM it, so `281 − 205` is positive in cooling; compare temperatures at idle, never during a transient. Every address 0–359 answers, so a response proves nothing and `registerCount` is link health. 184 addresses are published (the confirmed set, `P00`–`P139` as `raw.r<addr>`, and 215–217 as the `C16` defrost and `C17` freeze-protection candidates); address = parameter number is verified, but trust numeric matches over raw enum values. Register `65` reads 14 against the manual's `P65` = 20; confirm on the panel before reasoning about the low-flow trip.
- **The return-water target is 12 °C / 53.6 °F, set 2026-09-14 17:00 EDT as change 2 of `docs/chiltrix-cycling-tuning-plan.md` (10 °C before; `P12` widens to 3 next).** `P59` antifreeze trips at 3 °C on *leaving* water and the evaporator ΔT runs ~9 °F below the *return* target, so 49 °F is the floor. The controller stores whole °C, so set it in °C and read it back (48 °F becomes 8 °C, leaving water on the trip). E14 needs a breaker power cycle; the panel's Error reset does not clear it. To run colder: glycol → `P59` → target; `P109` only opens the range. If E14 recurs at 50 °F, check flow, charge and the leaving-water sensor, flow first. **The margin at 10 °C is zero at the end of a run and 3 °F in steady running**: every near miss since `P95` = 3 is a surge-terminated run (25 Hz to 47–60 Hz in the last one to three minutes, then stop; 5 % of runs, outlet 37.2–39.0 °F, 37.22 °F on 2026-09-08 with register 284 unmoved), while the 25 Hz floor holds 40.3–40.6 °F, so raising the target must precede any widening of `P12`. The start ramp is also fixed at 50–52 Hz by minute 3 whatever the load, which is why night runs are 11 min with 36 min gaps. Measured cooling COP is 4.9 all-in and 5.5–7.4 running (`docs/chiltrix-modbus.md`). Adding glycol does not help: `P59` trips on a fixed temperature and knows nothing about concentration. A full drain and refill follows `docs/hydronic-drain-and-refill.md`: pre-mix the whole charge at 30 % in a drum, fill and purge with a portable pump throttled against the boiler gauge (a dead-headed 65836 makes 52 psi against the 30 psi relief), and record the new glycol reading because it moves the `startupFlow` baseline.
- **`P5` is the low-flow alarm (`P65` trip, 20 L/min) and the Y-strainer is the first suspect**, fouled by scale from the boiler side of the shared loop. `C13` is the live flow readout; it reads 0 at idle because the pump only runs on a call, so read it in the 1–2 min pump-only window at the start of a run (54 L/min after cleaning). The failure signature is a chiller that runs a few minutes at a time while the loop warms, and, more quietly, sits above its own return target all afternoon. Never raise the autofill on this glycol loop; top up manually with premixed glycol to 21–23 psi. A Caleffi demineralisation filter has been on the fill side since 2026-09-19; makeup through it is water, so top-ups still use premixed glycol. The install dropped the hydronic pressure from 23.3 to 21.0 psi at the same tank temperature and left `.startupFlow` at 52.9 L/min.
- **The Y-strainer screen must be 60 mesh or finer (0.25 mm opening) per the CX65 and CX50-2 IOMs**, which tie the exchanger warranty to it; the exchanger's own blocking limit is about 1.2 mm (Alfa Laval CB class), so 20 mesh stops a blockage and 60 mesh stops progressive fouling and protects the flow meter. **The ACOL DN32 strainer's stock screen measures 60 mesh** (wire pitch 0.38–0.44 mm, 23–26 wires per cm, opening about 0.24 mm, measured against a millimetre tape on 2026-09-12), so it meets the IOM and needs no replacement on mesh grounds. The basket is 43.44 mm OD, 39 mm ID and 72.14 mm long, a stainless wire cloth backed by expanded metal, if one is ever ordered. **Fouling has slowed to nearly nothing**: 21 days after the 2026-08-22 cleaning the screen was about 5 % covered, against a full blockage in the first 18 days, so the loop is shedding its initial load and admitting no new scale; check again in a month. A finer screen fills faster, so pair it with a dirt separator upstream; `.startupFlow` is the sentinel either way.
- **Flow while running is a controlled output**, trimmed off inlet temperature (register 281) with the compressor, so raw flow, flow ÷ Hz, inlet against target, end-of-cycle inlet and run duration all fail as fouling signals. The **startup plateau** (`.startupFlow`, clean 51.7 L/min since the 2026-09-03 glycol top-up, 50.5–54 before it) is the one comparable measurement, and under 40 L/min is the alarm. **A glycol addition moves this baseline, so record the date of any top-up.** It is also blind to a *total* loss of flow — its 15 L/min floor discards a genuine zero, so `chiltrix-zero-flow` covers that. **The hydronic pressure step at pump start is a second fouling sentinel.** The tank separates flow, never pressure: the `.219` gauge sits on the suction side of the Chiltrix pump's path from the expansion tank connection, so `electrical.ac.arduinoThermPSI.psi` drops by the friction loss of the strainer, exchanger, tank tappings and piping between those points whenever the pump runs. Measured 2026-09-18 over 23 runs: −1.23 psi (−1.00 to −1.56) in the first minute or two at the 52.9 L/min plateau, climbing back about 0.5 psi as the chiller trims flow to 25–35 L/min, and the rest back at the stop; the drop follows flow squared (63 % of the day's pressure variance) and has no relationship to `IN`, `LBT` or the chiller inlet, so thermal contraction of the chilled glycol is at the noise level (−0.19 psi net per run). Read the step at the same startup flow; a growing step at a fixed flow is a filling screen. Baseline 1.0–1.6 psi with the screen about 5 % covered. **Register `284` is the fault/lockout word** (32 during the only observed lockout, 0 otherwise); no register latches a code, so the InfluxDB record is the only durable evidence a fault occurred. The chiller restarts on its own 2 °C hysteresis (`P12`). **`P52` = 2 since 2026-09-16 18:10 EDT**: the pump stops at target and runs one minute after each fifteen-minute stop, so zero idle flow becomes normal if the meter reads the stop, but it does not: the meter read 6.9 L/min through all 588 idle minutes of 2026-09-18 and never 0, the same trickle mode 0 held, so 6.9 L/min is the idle baseline the pressure step is measured against and a genuine zero is still a fault; `chiltrix-zero-flow` reads 7 minutes of zero flow as a fault and then needs a window that spans the restarts, and `.startupFlow` sees the restarts, so compare its baseline across the change. Any capacity analysis using data before 2026-08-22 is measuring a flow-restricted plant. Commissioned 2026-08-04; the first blockage took 18 days.
- **Shoulder-season heating (`docs/chiltrix-shoulder-season-heating-plan.md` §7–8):** a heat↔cool changeover moves the tank about 74 °F and costs 25–45 min of compressor time, 2.3 kWh to heat and 1.6 kWh to cool; the one measured so far (2026-09-08, partial swing) heated at 85 kBTU/h from 3.2 kW and cooled back at 0.034 kWh/°F with zones calling. A mains interruption shows as a sub-minute `operatingMode` = 1 blip with `HPHEAT` energised and a warm outlet; check `journalctl --list-boots` before suspecting the `B` terminal. **Winter (§9): the chiller stays on in heat mode all winter (decided 2026-09-15); the HZ-432 balance point (40 °F) sends calls below it to the boiler, the `DHWX` bridge feeds the zones from the tank whenever the boiler refuses a call for DHW, and the five hydronic thermostats sit on Heat, never Auto. Taking the unit out of service is HMI off with the breaker on, `DHWX` inhibit open, HZ-432 boiler-only (balance 50 °F or Emergency Heat).** Left on in heat mode it holds the tank at 122 °F for about 1,700 BTU/h at 60 °F ambient (measured 2026-09-15 with the primary off: 130.4 to 125.3 °F in 56 min, the pump idling at 8 L/min through the outdoor exchanger), about 16 reheat starts a day at 30 °F plus defrost, $100 for the winter, accepted; §9 prices it and the first cold week's record replaces the estimate. Chiltrix support (2026-09-16) concurs with `P52` = 2 for the winter (set 2026-09-16 18:10 EDT, `raw.r52` reads 2), expects no slush at 30 %, says `C17` freeze protection runs the pump at full speed and `C16` reports defrost, confirms register 143 is writable, regards cycle count and oil return as no concern, and advises a heating target as low as the rooms allow (the target stays at 50 °C for now). In off the unit stays powered for its standby protections, ignores the `C`/`H` contacts (the IOM's own instruction for relay-control wiring), keeps the off state through a mains outage (`P00` = 1), and still answers Modbus, so register 140 reads 0 and the freshness alerts stay quiet. Left enabled in cool mode it re-chills the tank at 0 °F ambient and courts E14; `chiltrix-cooling-cold` emails and raises `notifications.pivac.chiltrix_cooling_cold` when the unit is enabled in cooling mode with its own ambient reading under 40 °F for 30 min, which under option 2 means a stale cool call (thermostats on Auto). The HZ-432 has no cooling lockout by outdoor temperature (its `OT LOCKOUT` is a heating-stage lockout for non-dual-fuel panels) and the Chiltrix's outdoor-temperature standby (`P112`, `P42`/`P43`) cannot be used with `C`-`H`-`COM` control, which is why the thermostats must sit on Heat. 30 % glycol covers the outdoor exchanger for burst. The tank cannot cover a long DHW call, and the Ti-200's DHW priority time limit is the setting for that. At $1.93/therm and 18 ¢/kWh the chiller heats for less than the Ti-200 on every day down to about 35 °F (about $2/day saved through 40–55 °F), so the HZ-432 balance temperature is a capacity setting; measured boiler input is `1884 − 28.9 × T(°F)` kBTU/day from `scripts/boiler-heat-by-outdoor.py`, and the chiller COP is an estimate until the first heating week's energy balance replaces it.
...
- **DS2482 bring-up (EXT board, new Pi):** a chip that answers `i2cdetect` at `0x18` but logs `DS2482 reset failed` has VCC or ground open and is running on pull-up leakage; it passes at a 10 kHz I²C clock and fails at 50 kHz, so do not chase signal integrity — measure 3.3 V between chip pins 1 and 3 first (the bench fault was the GND rail not bridged to link position 5). Delete the `w1-gpio` line from config.txt rather than commenting it (`raspi-config` and a cloud-init `runcmd` both re-enable it) and confirm no `onewire` node in the device tree before reading any result. `ds2482-init.service` instantiates the master; the procedure is §8 of `docs/ds18b20-bus-topology.md`.
- **Bus health:** the DS2482 supplies the pull-up; no external resistor. **The EXT board hangs vertically below the Pi, solder side out, so the H plugs read GND · DATA · VCC left to right from the front**; that is the socket's V/D/G pin order seen from behind, and the label says so. **The trunk is CAT6 since the evening of 2026-09-07**, paired per rule 4 of `docs/ds18b20-bus-topology.md` (DATA with one GND, VCC with the other, spare pairs open at both ends), which replaced the flaky 22 AWG lever-connector pigtail. A swapped conductor at one end read on the DS2482 as DATA held low with phantom `Family 40` devices, then as an open once disturbed; eight probes with clean CRCs on every sweep is the proof it is right. Plug order at H1 is VCC · DATA · GND; with VCC and GND reversed the bus reads empty (line high, no presence, no short) and neither the probes nor the board were damaged by several minutes of it. **Read the chip's own status to split open from short from presence:** unbind the driver (`echo 1-0018 > /sys/bus/i2c/drivers/ds2482/unbind`), `i2cset -y 1 0x18 0xF0`, then `i2cset -y 1 0x18 0xB4; i2cget -y 1 0x18` and decode bit 1 = presence, bit 2 = short, bit 3 = line level; rebind afterwards. Line high with no presence is an open trunk or dead probes; presence with a count of 0 is marginal timing. `max_slave_count 64 reached` means marginal bit timing (cable capacitance against the pull-up), never 64 sensors. `presence=1 roms=0` means devices alive and timing marginal; `presence=0` means power or a broken lead. Bisect by adding probes one at a time while watching `w1_master_slave_count`. The Arduino bench rig needs 5 V on both supply and pull-up. pivac logs only `bus now has 0 sensor(s)`; the freshness alerts are what report a dead bus.
...
- **Mount a loop pair for ΔT, not for accuracy.** Copper at the tees, never PEX; probe axial and greased, 2–3 in thermal lead-in, ~10 pipe diameters of insulation each side, supply probe *after* the zone pump, and **strain relief an inch or two behind each probe**. Mount both probes of a pair identically so residual conduction error is common-mode and cancels out of the ΔT — an offset cannot remove a mounting bias.
...
- **An idle loop's ΔT is fiction**: the dead legs off the tees hold a plausible positive ΔT with the pump off, and loop A's supply tracks `IN` within 0.2 °F in every state. Gate every loop ΔT on a switch, never a probe (`pivac.LoopDelta` gates the primary on `HPCALL` or `DHWX` and each secondary on `HPCALL` narrowed to its zones: any heat or cool call on `zones`, a heat call only on `heat_zones`, which are the kitchen and great room on loop B because they cool on their own Bosch units; RedLink zone state lags the relay by ~48 s, and its fan-only state of 0.5 opens no gate). The first minute of a run is not a measurement either. Never aggregate a short-cycling signal into buckets near its own period.
...
- A run of `400 Client Error … /customers` every cycle while RedLink's warning rate stays normal is an Emporia cloud outage: it self-heals with no restart (2 h 40 min on 2026-09-16), the module logs in with the password every cycle until it does, and no Grafana rule watches `electrical.emporia.*`, so nothing announces the gap. Emporia's cloud keeps the minutes, so the gap is fillable afterwards with `scripts/emporia-backfill.py` (dry run, then `--write`; bound it by the last and first live points).
...
- **Inputs:** ZV 17, DHW 27, BLR 22, BOS2 5, BOS1 6, DEHUM 12, HPCALL 25, SCALA 23 (J3.2), HPHEAT 24 (J3.3), HPCOOL 13 (J4.2, `SP-C`), DHWX 19 (J4.3, `SP-D`). ZV is deliberately not plotted on the Relays panel. **`HPCALL` is the HZ-432's `Y1`** (named `CHIL` until 2026-09-15; InfluxDB history stays under `electrical.ac.switch.utility.CHIL`, and the rev A silkscreen and the J2.1 label still read `CHIL`): its poles run the Taco 0015 primary pump and enable the calling zones' secondary pumps on a heat-pump call of either kind. **`BLR` is the HZ-432's `W1`, read upstream of the boiler.** The Ti-200 serves DHW or space heat and never both, so a `W1` during a DHW call is refused at the boiler while `BLR` reads 1 (38 refused calls of 8 to 39 min in two spring weeks), and the CDP locks the zone valves out while `DHW` and `W1` call together so no cold water circulates through the coils. **The DHW bridge** (`docs/chiltrix-shoulder-season-heating-plan.md` §4.1, wired 2026-09-15) gates `W1` through a normally closed pole of the `DHW` relay and sends the refused call to the `DHWX` relay, whose contact closes the same Taco 503 input as `HPCALL`'s, so the tank feeds the zones for the length of the DHW call; `HPCALL` stays 0 through a bridged call and `DHWX` (spare pole on J4.3, BCM 19) is what marks it. It needs its own relay, since sharing the `HPCALL` coil would put `W1` on `Y1` and `Y1` on `W1`. **HPHEAT is the `HPHEAT` relay's spare pole** (since 2026-09-08): its coil is the HZ-432's `B` equipment terminal, held for as long as the panel is in heating mode; the relay's normally closed pole holds the Chiltrix's `C` contact closed and opens it while `B` is held, so 1 means the panel is in heat-pump heating mode. **HPCOOL is its twin** (since 2026-09-14): coil on the panel's `O` terminal, held for as long as the panel is in cooling mode, normally closed pole holding the Chiltrix's `H` contact and opening it while `O` is held, spare pole on BCM 13 (`SP-C`, J4.2), so 1 means the panel is in cooling mode, all summer, call or no call. `O` and `B` are changeover-valve outputs and the panel latches them by mode, so the chiller sees one contact open for as long as the panel is in a mode and keeps that mode, maintaining the tank between calls (IOM option 2, `docs/chiltrix-shoulder-season-heating-plan.md` §4); both closed only occurs with the panel in neither mode. `HPCALL` runs the Taco only. The `H`-`COM` pair is on `HPCOOL`, the `C`-`COM` pair on `HPHEAT`, nothing chained between the sockets, and `HPHEAT`'s normally open pole is empty. With one relay every satisfied heat call commanded cooling and re-chilled the tank. **SCALA is the AquaGuard AG-1250E leak sensor under the Scala booster pump** (since 2026-09-07): its 24 VAC normally-open output (WHITE, 1 A) feeds both a DLI IoT Power Relay control input, which drops the pump's normally-on outlets, and a 24 VAC relay whose contact closes J3.2 to J3.4, so `SCALA` = 1 means water in the pan and the pump already cut. The sensor is powered from the panel's 24 VAC on RED (hot) and BLACK (common); the IoT input and the relay coil sit between WHITE and common, about 0.1 A of the contact's 1 A. Adding a relay needs only `restart pivac-gpio`; the WilhelmSK SwitchBank lists the parent path, so a new name appears on every device by itself. **The tile's order is the `order` metadata on each switch path** (`SwitchBank.m getSwitchInfo`: meta `order`, else a `<switch>.order` value, else the key as an integer, which is 0 for every name here, so without metadata the order is whatever the app's dictionary yields). It is seeded in `~/.signalk/baseDeltas.json` as a `meta` update, ZV 1, BLR 2, DHW 3, HPHEAT 4, HPCOOL 5, HPCALL 6, BOS1 7, BOS2 8, DEHUM 9, SCALA 10, DHWX 11 (since 2026-09-15); a new relay needs an entry there and `restart signalk`, and the app needs a cold start to refetch metadata. The `inputs:` order in `config.yml` matches it for readability only. Zone → source in cooling: `HPCALL` serves MASTER_BR, DSTRS_FAM_ROOM and KIDS_ROOM; `BOS1` KITCHEN (`bova_kitchen`); `BOS2` GREAT_ROOM. **In heating all five zones are hydronic**: a KITCHEN or GREAT_ROOM heat call closes `HPCALL` and runs loop B (measured 2026-09-15, four great room calls with no master call, loop B probes swinging 8 °F), and the Bosch units only cool. The Bosch inputs report the *call* in parallel, never proof the compressor ran. **`HPCALL` is the call for the primary Taco pump, not for the chiller**: the chiller is always enabled and cycles on its own tank band, and the `HPHEAT` relay only selects its mode (cool on the normally closed pair, heat when energised; there is no off signal), so compressor runs are correlated with `HPCALL` rather than gated by it. Measured 2026-08-28 → 09-13: 10 of 390 runs had no `HPCALL` call before or during (tank maintenance, 0.14 kWh/day); the tank holds its temperature with the primary off and warms 30 °F/h with it on. Dropping or renaming a relay: config edit → `restart pivac-gpio` → `restart signalk`; no `.wlyt` change.
- **GPIO 26 (pin 37) is dead** (pad shorted since the 2026-06-23 power event), so its YOFF plateau Jun 23 → Jul 1 is fabricated. **The old bench Pi 4 `dc:a6:32:19:12:ee` has BCM 13, 16 and 25 dead** (found 2026-09-25: low under pull-up and pull-down with nothing on the header), so it cannot prove CHIL or SP-C on a board; BCM 16 only lacks its pull-up. **The bench Pi is now the new Pi 4 Rev 1.5 (`88:a2:9e:3c:c3:73`), all twelve channel pads good, and every rev A channel passed the guided walk on it on 2026-09-26** (`docs/rpi-io-boards-assembly.md`; run the walk detached with `--led` and the ACT LED reports each short from the bench). Every continuity check on an I/O board is made off the Pi, because the SoC's protection diodes tone every GPIO to ground. **YOFF is retired, not pending**: winter shutdown is a manual breaker-off at the chiller, so do not move the wire. Free input with a wire run: BCM 16; BCM 24 is `HPHEAT` since 2026-09-08, BCM 13 `HPCOOL` since 2026-09-14 and BCM 19 (`SP-D`, J4.3) `DHWX` since 2026-09-15, so the rev A board swap, which planned to move `HPCOOL` onto J4.3, needs `SP-C` or `SP-E` from the J8 pads for one of them. Suspected-stuck pin: disconnect, `raspi-gpio get` under `pu`/`pd`, then `op dh`; a pad the driver cannot raise is dead.

### RedLink (`docs/operational-notes.md`)

- **All five thermostats run 2 cycles per hour on both stages in heat and cool, and a 1 °F stage 2 differential (ISU 3030) in heat and cool, since 2026-09-17** (kids room and master bedroom cool rate since 2026-09-16; the stage 2 rate shows only on a thermostat configured two-stage). The cool rate was the default 3, which was producing 10-minute calls and 10-minute gaps at 50 % duty on the kids room; at 2 it makes calls of 12 to 22 minutes with gaps of 10 to 18 at the same duty. The rate has no say at full demand: a continuous kids room call (11 hours on 2026-09-16 at a 73 °F hold with 72–78 °F outside, the room holding 73 °F) is load, with precedent of 9 and 10 hours in the month before, and one degree of setpoint is worth about 20 points of evening duty (0.72 at 73 °F, 0.53 at 74 °F). Loop A ΔT on its calls follows the fan stage and the tank band (ΔT ÷ (room − supply) is 0.12 cycling and 0.16–0.22 at full demand; 2.0–2.5 °F on 55–57 °F supply, 3.6–4.9 °F on 50–52 °F), so compare ΔT only at like supply temperature and duty. Both are two-stage in heat and cool with stage 2 as the air handler's high fan, which the RedLink payload cannot show; staging is 3010 Advanced, 3020 No, 3030 1 °F, so the high fan means a room at least 1 °F off setpoint. 3030 offers only Comfort or whole degrees; under Comfort a continuous call held the high fan for hours with the display reading the setpoint (master bedroom four hours on 2026-09-17), and leaving the installer menu restarts the staging. The kids coil is sound (3.7 °F loop ΔT calling alone, zero droop at half duty), so cycling there is a thermostat setting and never a coil question; evidence in `docs/unico-cooling-assessment-and-tuning.md` Appendix J.
- **A humidity step in one zone only, alongside a domestic draw, is a shower**, never a coil or water-temperature finding: check `environment.water.domestic.flowRate` and the `DHW` relay (`electrical.ac.switch.utility.DHW.statenum`; every relay is stored under `.statenum` and `.state`, and the bare path returns nothing) for the 20 minutes before the step and the other four zones' humidity for a flat line. On 2026-09-17 the kids room went 52 → 59 % in ten minutes at 16:55 EDT at the end of an 18-minute draw, with `DHW` closed 16:40–17:15, while the other four zones held within a point. Through the eight hours before it the room held 51–54 % on 47–54 °F loop A supply, so the 12 °C target dries the room. Every dehumidification call reads as ordinary cooling in RedLink (`statenum` −1).
...
- **`vcgencmd get_rsts` tells a power loss from a watchdog reset.** `0x1000` is a power-on reset, which is what the 2026-09-07 19:40 EDT restart was: power bumped during the 1-wire rewire, which also opened the H1 pigtail joint (DS2482 status: line high, no presence) until the CAT6 re-pull replaced it that evening. A watchdog or software reset sets a different bit. The journal ends mid-line either way, because `RuntimeWatchdogSec=15` is set. Fetch InfluxDB analysis data one measurement at a time with `aggregateWindow` all the same; a wide `group()` + `pivot()` over a week of 1-minute data across ~25 series is the heaviest thing the Pi has been asked to do.
...
- **`navigation.position` is the house, seeded from `~/.signalk/baseDeltas.json`** (40.89929, −74.43873, added 2026-09-06), and the same file carries the relay `order` metadata for the WilhelmSK SwitchBank. Keep it: WilhelmSK 1.18 and 1.19 crash on every launch while a `notifications.mob` entry carries a null position (`SignalKSource.m:1028` subscripts an `NSNull`), and the server fills that field from `navigation.position`. The MOB button in the app's top bar exists only with "Use V2 Notification API" checked, which is also what makes silence, acknowledge and clear work on pivac alarms. If an MOB is raised by accident: `DELETE /signalk/v2/api/notifications/<id>` with an admin JWT, then `restart signalk` to drop the frozen path.
...
- **The reader publishes its own calibration health every cycle**: `hvac.boiler.sentry.decodeMargin` is the lit/unlit segment separation in grey levels (sound 40–60, under 20 is a knife edge) and `.decodeMisses` counts modes shown but not decoded. `sentry-decode-margin` fires on an hour's mean under 20, and any firing `source: sentry` rule makes the email bridge capture 300 frames, evaluate them under the live quad and mail the report with the eyecheck image (subject `[Sentry eyecheck]`, at most once per 30 min). The fix stays manual. **Choose a quad on margin, never on clean rate alone**: score candidates with `sentry-warp-search.py`, require 40 or more on every ±1 px neighbour, and calibrate on captures from both thermal states (soon after a DHW call and well after) whose digits between them include a `6` or `5` and a hundreds `1`. The 09-09 quad was 100 % clean on `7`, `1`, `0`, `8` and read every `6` as an `8` two days later, and the display does not move between the states: the content does. The module's segment rectangles sample the middle of each segment for this reason; widening them makes both states worse (`docs/sentry-cv-notes.md`, 2026-09-12).
- **Calibration drift recurs roughly monthly, and after every visit to the boiler room** (2026-07-28, 2026-08-23, a knock on 2026-09-07, creep plus a knife-edge quad on 2026-09-09/12, and a 4 px creep overnight after the 2026-09-12 strainer inspection that put all four LED spots on the bezel); any physical work in the boiler room can bump the camera, so check the decode rate afterwards. First check: `journalctl -u pivac-sentry | grep "nothing decoded"`. Recalibrate `display_warp` by measuring: capture ~400 frames, grid-search the quad offline, keep `dst_w`/`dst_h`/`digit_positions` fixed (the hundreds cell is a half digit), and validate against RedLink outdoor and the DHW recirc temperature, because a constant misread scores perfectly on consistency. Corners since 2026-09-13: `TL(1105,560) TR(1266,555) BR(1256,624) BL(1098,632)`, the search centre on a 300-frame Mac capture at margin 68. **A DHW call changes the digit content more than it moves the view**: the lit bounding box was identical across the cold and hot captures of 2026-09-12 while a creep of 2 px was measured on 2026-09-09, so a search must be run on captures from both thermal states and the quad set where the margin holds on both across its ±1 px neighbours; a taller quad is no substitute, it moves the segment rectangles into the neighbours' bloom. A knock moves the display further than `scripts/sentry-warp-search.py` searches (58 px left and 87 px up on 2026-09-07, with the image 2 % narrower and 14 % shorter): seed the quad from the lit-pixel bounding box of the digits and search translation with separate x and y scales. A large move is also quieter than a small one: with no indicator reading lit the module logs nothing and simply stops publishing `waterTemp` and `outdoorTemp`, so check for those paths going stale as well as for `nothing decoded`. **Since 2026-09-13 the reader tracks creep itself**: it matches the panel's fixed structure (digits masked) against a reference saved the first time the calibration read sound, and moves the quad and every lens spot by the measured shift on the next cycle, published as `registrationX`/`Y`/`Score`. The reference lives in `/var/lib/pivac/sentry-registration.npz` (the directory must be owned by `pi`) and resets whenever `display_warp`, `leds` or `indicators` change, so a manual recalibration is always the authority. The tracker follows translation only, up to 15 px at a score of 0.6 or better; a knock that also rescales the view, or a day/night flip, is held and logged, and still needs `scripts/sentry-warp-search.py`, which evaluates the creep-corrected quad. A border-lit drift detector was tried before this and does not discriminate per cycle.
- **`leds:` and `indicators:` drift with the camera too — re-aim them in the same pass as `display_warp`, never the quad alone.** They are absolute frame coordinates. Left behind through the 07-28 and 08-23 quad recalibrations, all eight slid off their lenses onto the bright bezel, which lifted `burner`'s dark reading to 1.040–1.051 against its 1.05 threshold and made `burnerOn` report noise: 371 samples lit while `gasInputValue` was 0, and 69 % of ON runs one sample long. Aimed coordinates since 2026-09-13 are LEDs `burner(1296,550) circ(1294,576) circ_aux(1291,603) thermostat_demand(1289,627)` and indicators `water_temp(1101,654) air(1154,650) gas_input(1204,648) dhw_temp(1254,649)`; a dark LED then reads ~0.76 and a lit one ~1.22. Locate all eight by dark-blob centroid on the per-pixel minimum of the capture (every lens is unlit somewhere in 600 frames), in a window no wider than ±12 px seeded by eye from a gridded zoom; a ±22 px window on the mean image put three LEDs on the bezel at 0.93–0.95 dark. **A drifted spot reads the bezel, not the LED**, so it barely moves when the LED lights — the old `burner` spot swung +0.012 across a real DHW call while the aimed one swung +0.421, and `circ_aux` there read the running pump as off for all ten samples. Check the swing, not the level. The module warns when a spot sits within 0.03 of its threshold for most of a cycle — grep `lit-threshold` alongside `nothing decoded`.
- **Do the frame captures from a Mac, not the Pi.** The camera is an ordinary LAN RTSP source, and capturing from the M2 works alongside a running `pivac-sentry` without stopping it or loading a fanless Pi 4. Crop at capture time and keep frames `uint8`: stacking 200 full-res float64 frames is ~5.9 GB and rebooted the Pi on 2026-09-06.
- **Validate `air` against `environment.outside.thermostat.temperature` (RedLink), not `environment.outside.temperature`** — the latter is a stale InfluxDB orphan that still answers `last(value)` with a months-old reading.
...
| `hvac.boiler.sentry.decodeMargin` | number | Lit minus unlit segment separation, grey levels; sound 40–60, under 20 alerts (`sentry-decode-margin`) |
| `hvac.boiler.sentry.decodeMisses` | number | Display modes shown this cycle but not decoded; emitted every cycle |
| `hvac.boiler.sentry.registrationX` / `.registrationY` | number | Camera creep in px the reader moved its quad and lens spots by this cycle (since 2026-09-13) |
| `hvac.boiler.sentry.registrationScore` | number | Match of the panel's fixed structure against the saved reference; 1.0 at the reference, held below 0.6 |
