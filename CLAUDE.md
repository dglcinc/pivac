...
- **RedLink: don't reset session on transient timeouts**: `status()` only calls `_reset()` for `AuthError`, `APIRateLimited`, or `SessionTimedOut` — not for plain `TimeoutError` or other transients. Earlier code reset on every exception, which forced a fresh ~75s login on every transient, producing visible 60–90s flicker in WilhelmSK during otherwise normal Honeywell flakiness. `_connect()` also splits login from discover so a discover-time timeout doesn't burn the auth session.
