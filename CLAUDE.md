...
- **Analysis queries against the Pi's InfluxDB go one measurement at a time.** A Flux `group()` + `pivot()` over nine days of 1-minute data across ~25 series coincided with a hard reset on 2026-09-07 19:40 EDT; `RuntimeWatchdogSec=15` is set, so a memory-starved influxd leaves no journal evidence, only a journal that stops mid-line. Fetch each series with `aggregateWindow` and join on the Mac. The reset also found the H1 pigtail joint open (DS2482 status: line high, no presence); the CAT6 re-pull that evening removed the joint.
