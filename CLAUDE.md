...
- **RedLink parallel refresh + per-device deadline**: Devices are refreshed concurrently via `asyncio.gather` and each `dev.refresh()` is bounded by `REFRESH_DEADLINE = 12s` via `asyncio.wait_for`. This is **independent of `request_timeout`** in config — that knob only governs the aiosomecomfort client used for login, which legitimately needs ~75s on Pi cold start. Don't merge the two timeouts; cutting `request_timeout` short causes `AuthError: Null cookie connection error 200` on every login attempt. A WARNING fires if any cycle exceeds 20s, so freshness regressions are visible at `--loglevel ERROR`.
