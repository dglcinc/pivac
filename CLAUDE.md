...
- **DS2482 bring-up (EXT board, new Pi):** a chip that answers `i2cdetect` at `0x18` but logs `DS2482 reset failed` is unpowered and running on pull-up leakage; it passes at a 10 kHz I²C clock and fails at 50 kHz, so do not chase signal integrity — measure 3.3 V between chip pins 1 and 3 first. Delete the `w1-gpio` line from config.txt rather than commenting it (`raspi-config` and a cloud-init `runcmd` both re-enable it) and confirm no `onewire` node in the device tree before reading any result. `ds2482-init.service` instantiates the master; the procedure is §8 of `docs/ds18b20-bus-topology.md`.
