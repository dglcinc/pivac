...
- **Inputs:** ZV 17, DHW 27, BLR 22, BOS2 5, BOS1 6, DEHUM 12, CHIL 25, SCALA 23 (J3.2), HPHEAT 24 (J3.3). ZV is deliberately not plotted on the Relays panel. **HPHEAT is the `HPHEAT` relay's spare pole** (since 2026-09-08): its coil is the HZ-432's `B` equipment terminal, energised on a heat-pump heating call; the relay's normally closed pole holds the Chiltrix's `C` contact closed at rest and opens it on a heating call, and its normally open pole closes `H` meanwhile, so 1 means the Chiltrix is heating the house. `CHIL` runs the Taco only. **Planned (`docs/chiltrix-shoulder-season-heating-plan.md` §4): a second relay `HPCOOL` on the panel's `O` terminal takes over `H` on its normally closed pole and reports on BCM 19 (`SP-D`), so both contacts are closed at rest and the chiller keeps its last mode between calls (IOM option 2)**; with one relay every satisfied heat call commands cooling and the tank is re-chilled. **SCALA is the AquaGuard AG-1250E leak sensor under the Scala booster pump** (since 2026-09-07): its 24 VAC normally-open output (WHITE, 1 A) feeds both a DLI IoT Power Relay control input, which drops the pump's normally-on outlets, and a 24 VAC relay whose contact closes J3.2 to J3.4, so `SCALA` = 1 means water in the pan and the pump already cut. The sensor is powered from the panel's 24 VAC on RED (hot) and BLACK (common); the IoT input and the relay coil sit between WHITE and common, about 0.1 A of the contact's 1 A. Adding a relay needs only `restart pivac-gpio`; the WilhelmSK SwitchBank lists the parent path, so a new name appears on every device by itself. **The tile's order is the `order` metadata on each switch path** (`SwitchBank.m getSwitchInfo`: meta `order`, else a `<switch>.order` value, else the key as an integer, which is 0 for every name here, so without metadata the order is whatever the app's dictionary yields). It is seeded in `~/.signalk/baseDeltas.json` as a `meta` update, ZV 1, BLR 2, DHW 3, HPHEAT 4, CHIL 5, BOS1 6, BOS2 7, DEHUM 8, SCALA 9 (since 2026-09-09); a new relay needs an entry there and `restart signalk`, and the app needs a cold start to refetch metadata. The `inputs:` order in `config.yml` matches it for readability only. Zone → source: `CHIL` serves MASTER_BR, DSTRS_FAM_ROOM and KIDS_ROOM; `BOS1` KITCHEN (`bova_kitchen`); `BOS2` GREAT_ROOM. The Bosch inputs report the *call* in parallel, never proof the compressor ran. **`CHIL` is the call for the primary Taco pump, not for the chiller**: the chiller is always enabled and cycles on its own tank band, and the `HPHEAT` relay only selects its mode (cool on the normally closed pair, heat when energised; there is no off signal), so compressor runs are correlated with `CHIL` rather than gated by it. Measured 2026-08-28 → 09-13: 10 of 390 runs had no `CHIL` call before or during (tank maintenance, 0.14 kWh/day); the tank holds its temperature with the primary off and warms 30 °F/h with it on. Dropping or renaming a relay: config edit → `restart pivac-gpio` → `restart signalk`; no `.wlyt` change.
- **GPIO 26 (pin 37) is dead** (pad shorted since the 2026-06-23 power event), so its YOFF plateau Jun 23 → Jul 1 is fabricated. **YOFF is retired, not pending**: winter shutdown is a manual breaker-off at the chiller, so do not move the wire. Free inputs with wire runs: BCM 13 (`SP-C`), 16, and 19 (`SP-D`, earmarked for `HPCOOL`); BCM 24 is `HPHEAT` since 2026-09-08. Suspected-stuck pin: disconnect, `raspi-gpio get` under `pu`/`pd`, then `op dh`; a pad the driver cannot raise is dead.
