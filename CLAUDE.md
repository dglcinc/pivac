...
- **A humidity step in one zone only, alongside a domestic draw, is a shower**, never a coil or water-temperature finding: check `environment.water.domestic.flowRate` (and the `DHW` relay) for the 20 minutes before the step and the other four zones' humidity for a flat line. On 2026-09-17 the kids room went 52 → 59 % in ten minutes at 16:55 EDT at the end of an 18-minute draw while the other four zones held within a point. Through the eight hours before it the room held 51–54 % on 47–54 °F loop A supply, so the 12 °C target dries the room. Every dehumidification call reads as ordinary cooling in RedLink (`statenum` −1).
