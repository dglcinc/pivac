...
| `DomesticWater` | DAE MJ-75a domestic water meter (UNO R4 WiFi, `10.0.0.188`). Wraps `pivac.ArduinoSensor` (one node GET) and passes through `environment.water.domestic.{flowRate,consumption,flowing,runDuration,runningFor}`, then **adds `.runVolume`** — gallons consumed during the *current* flow session, computed **Pi-side** (no firmware) by snapshotting the `consumption` totalizer on the `flowing` 0→1 edge and reporting the delta; resets to 0 when idle (like `runDuration`). State lives in the daemon (module globals). `run_s`/`runtime` (from the node) → `.runDuration`/`.runningFor` = this draw's duration for the WilhelmSK tiles. |
