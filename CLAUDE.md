...
- **OneWireTherm cascade-failure on bad sensor (known bug, unfixed)**: `pivac.OneWireTherm.status()` does NOT isolate per-sensor failures. If any one DS18B20 throws `SensorNotReadyError` mid-cycle, the exception bubbles up to `pivac-provider.py:171`'s module-level catch, the whole cycle is skipped, and NO 1-wire values publish — including from sensors that are perfectly healthy. Symptom: all three hydronic water-temp gauges (In/CRW/Out) go stale together even when only one sensor is bad. Confirmed live 2026-05-31: sensor `0516a365d8ff` (OUT) dropped off the w1 bus and silenced IN/CRW/AMB for ~7.5h. Fix shape: wrap the `sensor.get_temperature()` call inside the `for sensor in sensors` loop in a try/except (same isolation pattern as RedLink `_refresh_one`). Workaround until fixed: `sudo reboot` is the only reliable recovery — a w1_therm modprobe-reload + service restart will not help if the failing sensor still enumerates intermittently (service crash-loops on `NoSensorFoundError` at module import).
