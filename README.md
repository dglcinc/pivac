...
![Wilhelm SK dashboard](https://github.com/dglcinc/HVAC-pi/blob/master/WilhelmSK-dashboard-ipad.PNG?raw=true)
![Grafana dashboard](https://github.com/dglcinc/HVAC-pi/blob/master/Grafana-dashboard-ipad.PNG?raw=true)
...
Here's a photo of the finished install. The pipes on the left are the hydronic supply and return for one of the zones, with the zone valve bottom middle. The right side is a vertically mounted DIN rail. At the top of the rail are feed-throughs for all of the zones and power (kind of like a customizable punch-down block), using Phoenix Contact DIN rail mounted modular terminal blocks. In the middle are the Magnecraft relays (with timer and alternating relays at bottom to trim chiller runs), and at the bottom is the RPi in a commercially available DIN box (Phoenix Contact). On the left of the RPI is the header where the relay and 1-wire inputs go in. The RPi is powered by a 24VAC to 5VDC transformer (just visible under the header) that allows me to run the RPi off the standard HVAC power supply.

![HVAC-pi installation](https://github.com/dglcinc/HVAC-pi/blob/master/HVAC-pi.png?raw=true)
