...
The `hz432-mode-changeover` rule (added 16 September 2026) counts these: one email and one Signal K notification per changeover, with the Relays panel showing which zone called. The remedy, if the wrong water proves to matter, is a water-temperature interlock on the `ZV` relay's coil signal, the single point every valve call passes through, selected by mode through the spare poles of the two mode relays. Nothing already wired moves.

**The interlock is deferred on measurement (22 September 2026).** The record from 8 to 22 September holds four heating episodes (the evenings and mornings of 8, 15, 16 and 22 September), eight changeovers with a zone calling through the swing, and four sub-two-minute blips from mains events and a pair of 6-minute panel flips on 16 September that started two chiller heat runs and delivered nothing. Wrong-side water (under 80 °F on a heat call, over 70 °F on a cool call) reached a calling zone for 3 to 25 minutes per changeover, 12 to 18 minutes on the heat side from a 47 to 56 °F tank and 11 to 26 on the cool side from a 124 to 126 °F tank. Integrated from the primary ΔT at the energy-balance flows of the assessment, the heat delivered in the wrong direction was 0.4 to 6.6 kBTU per changeover and about 17 kBTU in all, which the chiller then removed again for about 1 kWh, under $0.20 for the fortnight. The worst case was 22 September at 10:21, when the family room called for cooling one minute after the panel dropped heat mode and took 115 °F water for 14 minutes. No thermostat moved during any wrong-water window: every zone that took wrong water held its reading at the 1 °F resolution RedLink reports, so the droop the interlock would prevent is below what the thermostats can see. The master bedroom drifted 71 to 75 °F in the hour after the 15 September 09:04 changeover and 71 to 73 °F after this one, but it was not calling either time, both were sunny mornings on that side, and the zone that took the hot water held steady.

Two things in the same record matter more. In three of the four episodes the kids room called for cooling while the master called for heat on the same loop, so loop A carried 60 to 110 °F water through the kids coil on a cool call; the water was right for the master, so the interlock does nothing for it, and the setpoint-spread rules are the remedy. And every full swing costs 25 to 45 minutes of compressor time whatever the valves do, so the changeover count is the number to watch. Revisit when the shoulder season produces daily changeovers, or sooner if a thermostat moves a full degree inside a wrong-water window or a changeover lands with two zones calling opposite ways. The analysis is a per-changeover table of the relay edges (`HPHEAT`, `HPCOOL`, `HPCALL`, and `CHIL` for the history before 15 September), `IN`, `OUT`, the tank and loop probes, the chiller registers and the five zones' RedLink state, at one-minute resolution; `IN` reads basement ambient while the primary is idle, so a threshold crossing counts only while a call is on.

**Parts.** One Honeywell L6006C1018 strap-on aquastat (SPDT, 65 to 200 °F, 5 to 30 °F differential, 8 A contacts) on the primary supply after the tank, set to about 70 °F with a 10 °F differential: `R`-`B` is closed while the water is under 70 °F and `R`-`W` while it is over 80 °F. It sells for $164 to $208. The Inkbird ITC-1000 (110 V version, $20) does the same job with two independent dry 10 A relay contacts and an NTC probe strapped to the pipe: with the target at 75 °F and both differentials at 5 °F its heating contact closes under 70 °F and opens at 75 (the `R`-`B` role) and its cooling contact closes over 80 °F and opens at 75 (the `R`-`W` role), and the two differentials are set separately, so the two-threshold refinement below costs nothing. It needs 120 V from a boiler-room outlet and an enclosure, and it carries no HVAC listing; a hung controller opens both contacts, which holds the valves on a heat-pump call while the boiler path still opens them, the safe direction. The Ranco ETC-111000 ($80 to $91) sits between the two and offers no saving. Gating the coil from the Pi's `IN` probe would be free and is ruled out: a pivac restart or a Pi outage would hold every zone valve, and the I/O boards have no output hardware.
